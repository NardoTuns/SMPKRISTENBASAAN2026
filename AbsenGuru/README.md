...
## 🚀 LANGKAH 2 — Tanamkan URL Web App ke Aplikasi (Sekali Saja)

Supaya guru **tidak perlu mengatur apa pun** dan tinggal buka aplikasi lalu masukkan password:

1. Buka file `assets/config.js` dengan editor teks apa saja (Notepad, VS Code, dll).
2. Isi baris berikut dengan URL Web App dari Langkah 1 (harus diakhiri `/exec`):
   ```js
   const DEFAULT_SERVER_URL = "https://script.google.com/macros/s/AKfycbxxxxxxxx/exec";
   ```
3. (Opsional) Ganti juga nama sekolah di `DEFAULT_SCHOOL_NAME` jika perlu.
4. Simpan file. Selesai — sebarkan folder aplikasi ini ke HP guru.

Setelah ini, layar "Masuk" akan langsung menampilkan kolom password saja (tab "Pengaturan
Server" otomatis disembunyikan). Jika suatu saat URL Apps Script berubah (misalnya deploy ulang),
admin bisa membuka link kecil **"⚙️ Pengaturan Server (khusus Admin)"** di bawah tombol Masuk
untuk mengubahnya langsung dari aplikasi, atau edit ulang `config.js` lalu sebarkan ulang.

> Jika `DEFAULT_SERVER_URL` dibiarkan kosong (`""`), aplikasi akan tetap meminta setiap
> perangkat mengatur URL server sendiri lewat tab "Pengaturan Server" seperti sebelumnya.

---

## 🚀 LANGKAH 3 — Jalankan Aplikasi SIABSEN
...
## 🚀 LANGKAH 4 — Login Pertama Kali (WAJIB Online)

1. Buka aplikasi di HP guru.
2. Jika URL server sudah ditanam di `config.js`, layar langsung menampilkan kolom **Password**
   — masukkan password yang tadi diatur di sheet Config (mis. `basaan2026`), lalu klik
   **"Masuk & Sinkronkan"**.
   (Jika URL belum ditanam, isi dulu lewat tab "Pengaturan Server" seperti biasa.)
3. Aplikasi akan mengunduh data guru & kop laporan dari Spreadsheet.
4. Setelah berhasil, aplikasi **bisa dipakai offline** kapan saja — password akan diverifikasi
